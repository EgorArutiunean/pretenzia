...
HELP_TEXT = (
    ...
    "Укажите дату претензии и срок оплаты, затем отправьте registry.xlsx. "
    "Обязательные колонки: Лицевой счет, ФИО, "
    "Адрес, Период задолженности, Сумма долга. Бот вернет claims.zip.\n"
    "\n"
    "4. Создать заявления в суд\n"
    "Сначала должны быть загружены Основная БЗ и Подсудность. Укажите дату "
    "заявления, затем отправьте "
    ...
    "только при рассчитанной сумме от 7 000 рублей. Максимальная сумма ограничивается "
    "36 месячными ставками. Пени пока не "
    "рассчитываются. Бот вернет court_orders.zip.\n"
    "\n"
    "Если в архиве есть errors.xlsx, обязательно проверьте все его листы "
    "перед использованием документов.\n"
    "\n"
